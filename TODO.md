...
- **WORK 갤러리 구현**: Pinterest 형태의 Masonry 그리드를 순수 CSS(`column-count`)로 구현 (1열~최대 6열 자동 팽창).
  ...
  - 모던한 박스 그림자 효과 및 dim/active 상태 UI 처리.
- **Window Z-Index 레이어 분리**: `1차 창`(`baseZ: 100`)은 무조건 `2차 창`(`baseZ: 10000`)의 뒤쪽 위계로 밀려 렌더링되도록 아키텍처 분리.
- **State Synchronization**: 2차 창이 외부적 이유(`X` 버튼 닫기 등)로 단독 종료될 경우에도, 연동된 갤러리 썸네일의 'is-active(활성 딤)' 상태가 즉각적으로 해제되도록 `windowClosed` 커스텀 이벤트 파이프라인 연결.
